...
var unmount = flag.Bool("unmount", false, "Unmount running filesystem and exit")
...
func start() *libfs.Error {
	...
	if *unmount {
		if err := dokan.Unmount(mountpoint); err != nil {
			return libfs.InitError(fmt.Sprint("Unmount failed: ", err))
		}
		return nil
	}
